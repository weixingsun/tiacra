...
static Handle<Value> Tiacra_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Tiacra.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TiacraBindings::lookupGeneratedInit(
...
static void Tiacra_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Tiacra_getBinding)->GetFunction());
}

static void Tiacra_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TiacraBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TiacraBinding = {
	"ti.acra",
	Tiacra_init,
	Tiacra_dispose
};
...
Java_ti_acra_TiacraBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.acra", &TiacraBinding);
	titanium::KrollBindings::addExternalLookup(&(::TiacraBindings::lookupGeneratedInit));
}
